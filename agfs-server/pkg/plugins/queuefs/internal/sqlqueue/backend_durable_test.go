...
func newSQLiteDurableTestBackend(t *testing.T, dbPath string) *Backend {
	t.Helper()
...
func TestSQLiteBackendDurableLifecycle(t *testing.T) {
	...
	if recovered, err := backend.RecoverExpired(queueName, time.Now().UTC().Add(2*time.Second), 0); err != nil {
		t.Fatalf("recover expired message: %v", err)
	} else if recovered != 1 {
		t.Fatalf("recovered count = %d, want 1", recovered)
	}
	if recoveredAgain, err := backend.RecoverExpired(queueName, time.Now().UTC().Add(2*time.Second), 0); err != nil {
		t.Fatalf("recover expired message twice: %v", err)
	} else if recoveredAgain != 0 {
...
func TestSQLiteBackendDurableRecoveryAfterRestart(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "queuefs-durable-restart.db")
	queueName := "jobs"
	now := time.Unix(1711600000, 0).UTC()
	...
	stats, err := backend.Stats(queueName)
	if err != nil {
		t.Fatalf("stats after restart recovery: %v", err)
	}
	if stats.Pending != 1 || stats.Processing != 0 || stats.Recoveries != 1 {
		t.Fatalf("stats after restart recovery = %+v, want pending=1 processing=0 recoveries=1", stats)
	}
	claimed, found, err := backend.Claim(queueName, model.ClaimRequest{})
	if err != nil {
		t.Fatalf("claim after restart recovery: %v", err)
	}
	if !found || claimed.MessageID != "restart" || claimed.Attempt != 2 {
		t.Fatalf("unexpected recovered restart claim: found=%v claimed=%+v", found, claimed)
	}
}
